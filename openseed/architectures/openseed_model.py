...
class OpenSeeD(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.size_divisibility = size_divisibility # default 32
        ...
        self.task_switch = {'coco': coco_on, 'o365': o365_on} # default coco
        print("self.task_switch ", self.task_switch)
        # HACK for only two datasets for seg and det
        if coco_on:
            task = 'seg'
            if not coco_mask_on: # default true -> X
                task = 'det'
            self.train_class_names[task] = get_class_names(train_dataset_name[0], background=background) # default coco_2017_train_panoptic
            ...
            for name in self.train_class_names[task]:
                names = name.split('-')
                if len(names) > 1: # if 'XX-OO'
                    assert len(names) == 2
                    train_class_names.append(names[1] + ' ' + names[0]) # => 'XX OO'
                else: # if 'XX'
                    train_class_names.append(name)
            self.train_class_names[task] = train_class_names

        if o365_on and len(train_dataset_name)>1: # default -> X
        ...
        if not self.semantic_on: # default true -> X
    ...
    def from_config(cls, cfg):
        ...
        if dec_cfg['TWO_STAGE']: # default true
            interm_weight_dict = {}
        ...
        backbone = build_backbone(cfg) #swin
        lang_encoder = build_language_encoder(cfg) #encoder
    ...
    def forward(self, batched_inputs, inference_task='seg'):
        ...
        if self.training: # default false
            losses = {}
    ...
    def forward_seg(self, batched_inputs, task='seg'):
        ...
        images = [(x - self.pixel_mean) / self.pixel_std for x in images] # normalized for each channel(RGB)
        images = ImageList.from_tensors(images, self.size_divisibility) # padding
